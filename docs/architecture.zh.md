...
私有终端渲染映射只在显式启用后，对可见、默认样式的普通 cell 添加有界语义色：HTTP
`2xx`/`3xx`/`4xx`/`5xx` 及常见的成功、信息、警告和错误状态词使用对应语义色。URL 和可操作的 Unix
路径不参与常驻覆盖，而由独立的 Cmd/Ctrl 目标交互负责短暂 accent 下划线。默认颜色从当前 Terminal
...
3. 用户明确确认后，控制器才原子持久化精确指纹。密码 profile 通过 Tokio blocking
   ...
   **Save password (optional)** 时才会在 profile 保存前更新所选后端。若请求加密保险库但未提供
   保险库口令，保存会为该 profile 生成随机解锁密钥并保存到应用私有目录的 `0600` 文件，SSH 密码仍保存在加密保险库中；密钥不会暴露给 UI，也不会通过系统密钥库读取。既有用户管理的保险库记录仍是保险库记录，解锁时仍需要
...
4. Settings > General 持有新记住 SSH 密码的默认后端：默认是应用加密保险库，也可以显式选择平台系统凭据库。
   ...
   **Save password (optional)** 时，内嵌密码只供 **Save & connect** 使用一次，单独保存会丢弃该密码。
   勾选后才把密码与 profile 事务性写入所选后端；缺少保险库口令时会把随机隐藏解锁密钥保存到应用私有目录，不会把 SSH 密码后端改为系统凭据库，也不会创建无法解锁的保险库记录；
...
若选择加密保险库但未提供保险库口令，保存会为该 profile 生成并保存到应用私有目录的 `0600` 文件，不会把 SSH 密码后端改为系统凭据库，也不会创建空口令保险库记录；既有用户管理的保险库记录仍需要其保险库口令才能解锁。Settings > General 初始化以后保存密码时使用的
后端：系统后端使用 macOS Keychain、Windows Credential Manager 或 Unix Secret Service；保险库
后端使用按 profile 分隔的应用记录；自动生成的保险库密钥随机、按 profile 分隔，并单独保存到应用私有目录。默认本机自动解锁不需要再次访问系统密钥库，但同一操作系统账户下有文件访问权限的进程仍可读取该解锁文件。保险库对每条记录用 Argon2id 派生密钥、用
...
拥有自己的字体、字号、行高、
文字亮度、粗体亮色、可选语义高亮及其状态颜色覆盖和终端交互设置。两个字体列表都固定先显示自带字体，随后显示由 `fontdb` 在
...
发行自动化只拥有发行元数据，不拥有应用运行时状态。发布者在默认分支创建 annotated 日期 tag：当天首发
公开 tag 为 `YYYY-MM-DD`，正整数修订后缀形成 `YYYY-MM-DD-N`。基础日期映射为 Cargo/Debian 的
`YYYY.M.D` 和 macOS build 的 `YYYYMMDD`；
修订映射为 Cargo `YYYY.M.D+N`、Debian `YYYY.M.D-N` 和 macOS build `YYYYMMDD.N`，macOS short
version 保持 `YYYY.M.D`。`scripts/release_version.py sync` 会在提交前更新锁文件和 macOS bundle 元数据。
匹配候选模式 `20*-*-*` 的直接 tag push 会启动唯一的 Release workflow。它先校验 ref 是 annotated tag，
...
`settings` 对象，包括分别经过约束的应用字体与 Terminal 字体、终端字号、行高、文字亮度、粗体亮色、可选语义高亮及其状态颜色覆盖和鼠标复制/粘贴偏好、
...
因为二者不存在安全数值映射；已保存的语义覆盖色会保留，但启用前不生效。schema 版本 21 新增跟随系统的界面语言策略；缺失或无效值默认跟随系统。schema 版本 20 增加五项可选 Terminal 语义色字段；空值或无效值跟随活动 ANSI 色表，非空值规范化为不透明 `#RRGGBB`。历史版本中的 link/path 字段仍保留以兼容配置，但不再参与 renderer 或 Settings 表面。schema 版本 19 增加 SSH SFTP 默认目录字段；缺失远端值
...
显式启用时，有界语义覆盖层只改变符合条件的可见默认样式 cell，并从当前终端色表派生不同的成功、信息、警告和错误色；存在规范化的 Settings 覆盖时使用该颜色。URL/路径保持终端原有前景色，只在按住 Cmd/Ctrl 时显示临时下划线。
...
使用临时密码、本机私钥或有界运行时 SSH agent 完成 SSH 认证，为已认证 SSH Tab 提供有界远端 SFTP、本地元数据目录浏览和下载到所选 Local files 目录的 regular file，并持有多个逐 Tab 隔离的 transport 或本地 shell
