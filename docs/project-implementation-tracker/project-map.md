...
| `src/app.rs` | Slint/Rust bridge 入口与原生窗口路由 | `slint::include_modules!`、`run`、`wire_callbacks`、`WindowRouter`、`WorkspaceTransfer`、About support actions | 生成类型声明、共享 event loop、多 `AppWindow` 生命周期、按 Tab 的 detached/main route、直接 Tab UUID 的 Window Move/Return callbacks、启动级资源边界、固定平台 Tab accelerator、功能 callback 和脱敏诊断复制/外部路径打开；无 SFTP 启动扫描 |
...
| `src/app/connection.rs` 与 `src/app/connection/` | 多协议连接控制器组装入口与单一流程模块 | `wire_connection_request`、`request_profile_connection`、`begin_authentication`、`start_telnet_connection`、`start_serial_connection`、`start_session_worker` | `request` 解析 SSH/SFTP companion 导航、分发协议并把编辑器一次性密码绑定到对应 Tab；`authentication` 让 agent profile 绕过密码加载/弹窗并直接启动 worker；`direct` 负责 Telnet/Serial attempt 与 monitor；其余模块保持 SSH probe、信任、认证和 worker phase；未知或变化密钥仍须明确确认 |
...
| `src/app/view.rs` | Slint model/snapshot/主题映射 | `refresh_workspace`、`apply_sftp_snapshot`、`prewarm_file_icons`、`load_private_key_options`、`apply_active_snapshot`、`apply_security_prompt`、`apply_theme_to_component` | 按 WindowRouter 过滤 workspace Tab 与 active snapshot；SFTP 图标/transfer DTO、终端渲染、font/private-key options、Light/Dark 主题、活动 Tab 安全覆盖层、blocking 快照更新，以及去重/有界且带 generation 的图标与私钥发现调度 |
...
| `src/app/state.rs` | 工作区 Tab、窗口 transfer、终端、SFTP transfer、编辑器 draft、transport route 与 SSH phase 所有权 | `AppState`、`WorkspaceTransfer`、`SftpBrowserState`、`SftpTransferState`、`cycle_tab_for`、`workspace_transfer_for`、`switch_ssh_sftp_tab`、`TerminalWorker`、`SshConnectionPhase` | Tab/attempt 生命周期、SSH/SFTP companion UUID 成组转移、逐 Tab 有界 transfer history/状态、worker/终端 resize、短期认证秘密和编辑草稿；窗口 transfer 只含 UUID，不持有 russh handle/receiver 或持久化 transfer |
...
| `ui/app.slint` | Rust-facing 主窗口/独立窗口、菜单栏和顶层 Slint 转发 | `AppWindow`、`WorkspaceViewState`、`SecurityOverlayViewState`、`MenuBar` | Rust property/callback 接口、共享 event loop 的多窗口标记、detached 时以活动连接名作为原生标题、仅主窗口的 File/Window 菜单、直接 Tab UUID 的 Move/Return workspace、DTO 组装、主题刷新和安全 phase 输入；不在此保存局部草稿或普通弹层状态 |
| `ui/workspace-shell.slint` | 工作区局部组合和短暂 UI 状态 | `WorkspaceShell`、`DetachedWorkspaceContent`、`WorkspaceViewState` | 主窗口侧栏/连接选择器/Settings 与会话编辑器、仅 Terminal/SFTP 的 detached 内容组合、SFTP 运行时分栏比例/Transfers 折叠、Tab/内容 callback 转发；接收只读 Profile/Tab/终端/设置快照 |
