...
| `src/app/macos_window.rs` | macOS 原生窗口/菜单 bridge | `configure`、`configure_detached_return_button`、`current_modifier_state`、`configure_application_menu`、`NativeMenuTarget` | 标准标题栏、带系统符号/模板回退及 Tooltip/无障碍描述的 detached 同行返回图标、当前物理修饰键，以及扫描当前 AppKit 菜单树、兼容省略号并带实时 key equivalent 的 Settings/About 幂等绑定与主线程生命周期 |
...
| `ui/components/sftp-controls.slint` | 可复用 SFTP 操作按钮 | `SftpActionButton` | 有界高度、键盘/无障碍 button 语义与 SFTP 视觉 token；不保存 SFTP 状态或执行 I/O |
...
- 修改终端解析、scrollback、pane 或 resize：先检查 `src/app/panes.rs` 的最多 8 leaf、方向和 collapse 语义，再检查 `src/terminal.rs`；主屏使用 `alacritty_terminal::Term::resize` 的重排语义，备用屏不重排。UI resize 只从 `src/app/terminal_bridge.rs` 带 `tab_id` 进入 `AppState::resize_terminal(tab_id, ...)`，由状态层请求指定 worker 并更新指定本地模型；worker/Slint 只消费既有 `TerminalSnapshot`。拆分只能新建独立 local PTY 或 profile connection，SSH child 不得继承短期秘密。修改按键序列：`src/terminal/input.rs`，并同步 `src/app/input.rs`、`ui/{terminal-pane,workspace-shell}.slint` 与 `TerminalSettings::option_as_meta`；修改终端网格绘制、指针或菜单意图时还需检查 `ui/components/terminal-grid.slint`，其中完整行向下对齐且顶部余量必须同步 IME/预编辑和指针坐标。SSH shell I/O 检查 `src/ssh/worker.rs`；本地 PTY I/O 与 shutdown 检查 `src/local_shell.rs`，保持进程组/child 终止、reader 和 worker join 的单一所有权。
...
- 最近依据：2026-08-11 schema v19 为 SSH profile 增加有界、非秘密的 `sftp_remote_path` 与 `sftp_local_path`；Session Editor 通过只读 DTO/局部草稿编辑，保存后只在新 SFTP Tab 初始化时生效。worker 取得远端初始路径，`AppState` 取得本地初始路径；缺失远端值回退 `~`，空本地值回退平台 home，既有 Tab、凭据和 host-key trust 不受影响。
