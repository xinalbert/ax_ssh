...
| `src/app.rs` | Slint/Rust bridge 入口与原生窗口路由 | `slint::include_modules!`、`run`、`wire_callbacks`、`WindowRouter`、`WorkspaceTransfer`、About support actions | 生成类型声明、共享 event loop、多 `AppWindow` 生命周期、可见 Tab 与焦点 pane 身份、每窗口多组 PaneTree、detached/main route、整组关闭/Return、启动级资源边界、固定平台 Tab accelerator 和脱敏诊断；无 SFTP 启动扫描 |
| `src/app/panes.rs` | 有界 Terminal Tab 内 pane 布局 | `PaneTree`、`PaneDirection`、`PanePlacement`、`MAX_TERMINAL_PANES` | 稳定 workspace Tab UUID、终端分行/分列、相邻方向焦点和最多 8 pane 限制；只保存 UUID、布局和焦点，不保存 Slint、worker、buffer 或秘密 |
...
| `src/app/terminal_bridge.rs` | 终端与本地 shell bridge | `wire_terminal`、`start_local_shell`、`spawn_local_shell_monitor` | 仅在 connected 后接收 UUID 定向 terminal 输入/selection；处理 pane focus/split、独立 local PTY 创建和方向快捷键；resize 只进入 `AppState::resize_terminal(tab_id, ...)` |
...
| `src/app/view.rs` | Slint model/snapshot/主题映射 | `refresh_workspace`、`apply_terminal_panes`、`apply_sftp_snapshot`、`prewarm_file_icons`、`load_private_key_options`、`apply_active_snapshot`、`apply_security_prompt`、`apply_theme_to_component` | 按 WindowRouter 过滤 workspace Tab 与 active snapshot；将每个 pane 的有界终端 snapshot 映射到 Slint DTO；SFTP 图标/transfer DTO、font/private-key options、Light/Dark 主题、活动 Tab 安全覆盖层、blocking 快照更新，以及去重/有界且带 generation 的图标与私钥发现调度 |
...
| `src/app/state.rs` | 工作区 Tab、窗口 transfer、终端、SFTP transfer、编辑器 draft、transport route 与 SSH phase 所有权 | `AppState`、`WorkspaceTransfer`、`PaneSessionSource`、`resize_terminal`、`workspace_transfer_for_terminal_panes`、`SftpBrowserState`、`SftpTransferState`、`move_tab_for`、`switch_ssh_sftp_tab`、`TerminalWorker`、`SshConnectionPhase` | Tab/attempt 生命周期、隐藏 pane session 与可见 Tab 排序、SSH/SFTP companion UUID 成组转移、按 UUID 的 worker/terminal resize、拆分 session source、短期认证秘密和编辑草稿；窗口 transfer 只含 UUID |
...
| `src/sftp/transfer.rs` | 远端 regular file 只读下载与私有 cache | `SftpDownloadRequest`、`SftpDownloadHandle`、`cleanup_stale_sftp_open_cache` | 512 MiB、64 KiB chunk、metadata 重验、bounded writer/progress、取消/超时、`.part` fsync/atomic rename、权限和首次下载前 4096 项维护 |
...
| `ui/app.slint` | Rust-facing 主窗口/独立窗口、菜单栏和顶层 Slint 转发 | `AppWindow`、`WorkspaceViewState`、`TerminalPaneView`、`SecurityOverlayViewState`、`MenuBar` | Rust property/callback 接口、共享 event loop 的多窗口标记、detached 时以活动连接名作为原生标题、仅主窗口的 File/Window 菜单、直接 Tab UUID 的 Move/Return/terminal callbacks、DTO 组装、主题刷新和安全 phase 输入；不在此保存局部草稿或普通弹层状态 |
| `ui/workspace-shell.slint` | 工作区局部组合和短暂 UI 状态 | `WorkspaceShell`、`DetachedWorkspaceContent`、`TerminalPaneGroup`、`TerminalPaneView`、`WorkspaceViewState` | 主窗口侧栏/连接选择器/Settings 与会话编辑器、仅 Terminal pane/SFTP 的 detached 内容组合、UUID 定向 terminal callback 转发、SFTP 运行时分栏比例/Transfers 折叠、Tab/内容 callback 转发；接收只读 Profile/Tab/终端/设置快照 |
...
- 最近依据：2026-08-10 将 macOS detached 标题栏 Return 文字按钮改为紧凑的系统返回图标，保留 Tooltip、无障碍描述、AppKit 模板回退及原有路由 callback；同日将 Terminal 分屏收紧为“一棵 PaneTree 属于一个可见 Terminal Tab”，子 pane 保留独立 UUID/worker/model/认证阶段但不进入 Tab 条，窗口路由分别维护可见 Tab 与焦点 pane，Tab 循环/拖动/关闭及 detached/Return 按整组处理，SFTP companion 仍为独立可见 Tab。
