...
| `src/app.rs` | Slint/Rust bridge 入口 | `slint::include_modules!`、`run`、`wire_callbacks`、About support actions | 生成类型声明、UI 启停、固定平台 Tab accelerator、功能 callback 和脱敏诊断复制/外部路径打开 |
...
| `src/app/state.rs` | 工作区 Tab、终端、编辑器 draft、transport route 与 SSH phase 所有权 | `AppState`、`cycle_tab`、`switch_ssh_sftp_tab`、`SshSftpNavigation`、`resize_active_terminal`、`TerminalBackend`、`TerminalWorker`、`TerminalTabState`、`SshConnectionPhase` | Tab 创建/前后循环/切换/关闭、运行时 companion UUID 配对与邻接插入、同 profile 多实例、逐 Tab attempt、活动 worker 与本地模型 resize 编排、SSH 安全 phase、对应 Tab 的短期一次性认证秘密和编辑草稿身份；配对与秘密都不持久化，也不共享 live transport |
...
| `src/app/sftp_bridge.rs` | SFTP Tab 浏览与本地目录 intent bridge | `wire_sftp` | 仅对活动且 connected 的 SFTP Tab 转发远端目录/分页、前进后退和选中意图；不持有协议 session |
...
| `src/ssh/worker.rs` | SSH/SFTP-only worker 生命周期 | `SshSessionHandle`、`SshSessionMode`、`SshSessionEvent`、`flush_output` | 有界 shell/SFTP-only 连接；认证前忽略普通操作且只由 Disconnect/controller 取消；仅在服务端打开 X11 channel 时开始本机准备和 relay；即时输入、输入后首个输出刷新、脱敏 latency、退出 join 和短期秘密 |
