...
| `src/app.rs` | Slint/Rust bridge 入口与原生窗口路由 | `slint::include_modules!`、`run`、`wire_callbacks`、`WindowRouter`、`WorkspaceTransfer`、About support actions | 生成类型声明、共享 event loop、多 `AppWindow` 生命周期、可见 Tab 与焦点 pane 身份、每窗口多组 PaneTree 与 divider ratio、detached/main route、整组关闭/Return、启动级资源边界、固定平台 Tab accelerator 和脱敏诊断；无 SFTP 启动扫描 |
...
| `ui/app.slint` | Rust-facing 主窗口/独立窗口、菜单栏和顶层 Slint 转发 | `AppWindow`、`WorkspaceViewState`、`TerminalPaneView`、`TerminalPaneDividerView`、`SecurityOverlayViewState`、`MenuBar` | Rust property/callback 接口、共享 event loop 的多窗口标记、detached 时以活动连接名作为原生标题、仅主窗口的 File/Window 菜单、直接 Tab UUID 的 Move/Return/terminal/divider callbacks、DTO 组装、主题刷新和安全 phase 输入；不在此保存局部草稿或普通弹层状态 |
| `ui/workspace-shell.slint` | 工作区局部组合和短暂 UI 状态 | `WorkspaceShell`、`DetachedWorkspaceContent`、`TerminalPaneGroup`、`TerminalPaneDivider`、`WorkspaceViewState` | 主窗口/独立窗口共用的 pane/divider 组合、可访问拖拽/键盘 resize、UUID 定向 terminal callback 转发、SFTP 运行时分栏比例/Transfers 折叠、Tab/内容 callback 转发；接收只读 Profile/Tab/终端/设置快照 |
