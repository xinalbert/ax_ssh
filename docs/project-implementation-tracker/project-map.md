...
| `src/app/terminal_render.rs` | UI 独立的终端色表、最小对比度和有界语义色映射 | `render_terminal`、`SemanticPalette`、`semantic_highlights` | 终端 ANSI/256/真彩色、默认主题背景和选择色，以及仅默认 ASCII run 的 URL/路径、HTTP 状态、成功/警告/错误标记；不访问 Slint、worker、持久化或终端 buffer |
...
| `src/config.rs` 与 `src/config/` | 稳定 config 入口及 session、settings、theme、persistence、回归测试模块 | `ConnectionProfile`、`SessionProfile::validate`、`SessionStore::validate`、`ConfigStore`、`MAX_CONFIG_FILE_BYTES`、profile/store 共享上限 | schema v19 增加 SSH profile 的 SFTP 默认远端/本地目录（旧值回退 `~`/平台 home），v18 新增默认关闭的终端选中即复制偏好，v17 终端最小对比度替换旧亮度字段；profile 字段拒绝控制字符并限制长度，store 限制 1,024 profiles/256 groups/8 MiB；私有写入使用同目录唯一 `create_new` 临时文件、失败清理和原子替换；SSH-only 安全字段由 variant 隔离 |
...
- 最近依据：2026-08-12 Terminal render mapper 对每个有界可见行的默认 ASCII run 做边界感知语义标注：URL/路径、HTTP 2xx-5xx 和常见成功/警告/错误词分别使用当前 ANSI 色表的 link/success/warning/error 色，并对当前终端背景校正至至少 4.5:1。显式 ANSI/真彩色、非默认背景、inverse、dim 与非 ASCII run 不改变；不向 Slint、worker 或持久化层传递额外终端内容。
