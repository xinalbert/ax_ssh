...
| `src/app.rs` | Slint/Rust bridge 入口与原生窗口路由 | `slint::include_modules!`、`run`、`wire_callbacks`、`WindowRouter`、`WorkspaceTransfer`、About support actions | 生成类型声明、共享 event loop、多 `AppWindow` 生命周期、按 Tab 的 detached/main route、每窗口 volatile pane tree、直接 Tab UUID 的 Move/Return/terminal callbacks、启动级资源边界、固定平台 Tab accelerator、功能 callback 和脱敏诊断复制/外部路径打开；无 SFTP 启动扫描 |
| `src/app/panes.rs` | 有界窗口内终端 pane 布局 | `PaneTree`、`PaneDirection`、`PanePlacement`、`MAX_TERMINAL_PANES` | 终端分行/分列、相邻方向焦点、关闭 leaf 后折叠和最多 8 pane 限制；只保存 UUID、布局和焦点，不保存 Slint、worker、buffer 或秘密 |
...
| `src/app/workspace.rs` | 工作区与 profile/group bridge | `ProfileMutationCoordinator`、`wire_workspace_tabs`、`wire_session_editor`、`wire_session_management`、`SessionTransferEnvelope`、`import_session_transfer_into_store`、`close_workspace_tab` | Tab 激活/前后循环/内存排序、pane close 后 route collapse、会话 CRUD、逐 profile 最新 mutation token、串行凭据副作用与过期回滚、编辑器一次性密码/显式保存分流、Group/Server Duplicate、256 KiB/128 profile 脱敏 JSON 复制导入和关闭时资源回收 |
...
| `src/app/macos_window.rs` | macOS 原生窗口/菜单 bridge | `configure`、`configure_detached_return_button`、`current_modifier_state`、`configure_application_menu`、`NativeMenuTarget` | 标准标题栏、detached 同一行标题栏 Return、当前物理修饰键，以及扫描当前 AppKit 菜单树、兼容省略号并带实时 key equivalent 的 Settings/About 幂等绑定与主线程生命周期 |
| `src/app/state.rs` | 工作区 Tab、窗口 transfer、终端、SFTP transfer、编辑器 draft、transport route 与 SSH phase 所有权 | `AppState`、`WorkspaceTransfer`、`PaneSessionSource`、`resize_terminal`、`workspace_transfer_for_terminal_panes`、`SftpBrowserState`、`SftpTransferState`、`cycle_tab_for`、`switch_ssh_sftp_tab`、`TerminalWorker`、`SshConnectionPhase` | Tab/attempt 生命周期、SSH/SFTP companion UUID 成组转移、按 UUID 的 worker/terminal resize、拆分 session source、短期认证秘密和编辑草稿；窗口 transfer 只含 UUID，不持有 russh handle/receiver 或持久化 transfer |
