...
3. 保存会话并在会话导航中选择它。新建 SSH profile 时，也可以点击 **Save & connect**，
   ...
   AxSSH 会生成逐服务器的随机隐藏解锁密钥，将其保存到应用私有目录的 `0600` 文件；该密钥只用于后台解锁，绝不会显示，正常保存和启动不会访问系统密钥库。重复打开同一个已保存会话会创建彼此独立的终端
...
5. 使用 SSH 密码或私钥认证时，根据提示输入临时密码或私钥 passphrase；私钥 passphrase 永远不会持久化。若要记住
   ...
   **Encrypted application vault**。密码弹窗会以该设置为初始值，也可以在 **Credential
   ...
   如果攻击者能够观察签名时序，请不要使用 RSA 认证；条件允许时优先使用 Ed25519 或 ECDSA。
...
选择加密保险库但保险库口令留空时会生成隐藏的逐服务器解锁密钥并保存到应用私有目录。
...
平台 home 目录，且只读取有界文件元数据。点击 **Name**、**Size** 或 **Modified** 可切换文件列表排序，
默认按修改时间最新优先。
**Hidden** 显示点文件；**More** 释放本地下一页（每页 250 条）或请求远端下一页目录。
过长名称会按列宽省略，鼠标悬浮在被省略的名称上即可看到完整名称，提示内容会自动换行。
文件行优先显示目标平台提供的文件类型图标；平台无法解析时使用内建的目录、链接或通用文件图标。
...
AxSSH 在布局变化稳定后自动保存，并在退出时再次保存到独立的私有 `workspace.json`。
重新启动后会恢复主窗口/独立窗口的位置、大小及最大化状态、Tab 顺序、活动 Tab、终端分屏
结构与比例、有界终端文本以及 SFTP 浏览路径。不会自动进入全屏或最小化；副屏拔出后窗口
会调整到当前可见屏幕，Wayland 的绝对定位由系统管理。停止移动/调整窗口约一秒后自动保存，
强制退出仍可能丢失尚未写盘的最后变化。
...
八条。下次启动优先恢复私有 `workspace.json`；只有快照不存在或无效时，才依次尝试最近
文件，并移除不可用记录。自动保存不会覆盖命名工作区文件，更新它们需要使用 Save Workspace。
**Clear Recent** 只清除路径历史。
...
仍只打开会话编辑器。连接或打开新编辑器时，键盘焦点会同步到当前工作区 Tab，并自动滚动至
可视区域；随后编辑器会将焦点落在**会话名称**字段。工作区 Tab 也可通过 Tab 键导航，按 Enter 或 Space 激活
当前获得焦点的 Tab。
在不同主机对应的工作区 Tab 之间切换时，侧栏会选中并高亮对应的服务器行；切换到非主机
Tab 时保留侧栏当前选择，终端和编辑器的键盘输入焦点不会被侧栏抢走。
...
选中的会话可以直接接收下一次按键。
从其它应用或 AxSSH 窗口返回时，也会自动恢复活动 Terminal Tab 的输入焦点；
打开阻塞式对话框时不会抢占该对话框的输入。
...
配置目录中为每个 profile 保存一个私有加密记录。保险库口令留空时，随机解锁密钥会单独保存到应用私有目录的 `0600` 文件且不会显示；用户输入的保险库口令不会保存。这样可避免额外的系统密钥库口令提示，但同一操作系统账户下有文件访问权限的进程仍可读取解锁文件。每日日志单独写入
