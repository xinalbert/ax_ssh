...
- 最近一次完整 locked/offline 测试门禁通过：库测试 141 项、应用测试 127 项和 Doc tests 0 项均无失败；Terminal divider drag/focus 修复已通过 `cargo check --locked --offline` 与 2 项 model identity 定向测试。本轮没有新增 crate、版本升级、`Cargo.toml`、`Cargo.lock`、配置 schema、工具链或 CI 契约变化。Cargo fmt/Clippy 子命令仍缺失，直接 `rustfmt` 是本机格式回退；目标平台连续拖动、焦点恢复、键盘/无障碍 divider 和真实 PTY resize 仍需人工确认。
