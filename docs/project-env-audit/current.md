...
- 最近一次完整 locked/offline 测试门禁通过：库测试 141 项、应用测试 125 项和 Doc tests 0 项均无失败；Terminal Edit 菜单已通过直接 Rustfmt、`cargo check --locked --offline`、tracker validator、46 个 Markdown 相对链接和 `git diff --check`。本轮没有新增 crate、版本升级、`Cargo.toml`、`Cargo.lock`、配置 schema、工具链或 CI 契约变化。Cargo fmt/Clippy 子命令仍缺失，直接 `rustfmt` 是本机格式回退；三平台原生菜单显示、focused pane 定向和 detached 快捷键仍需目标平台人工确认。
